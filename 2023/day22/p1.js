...
fs.readFile('./input.txt', 'utf8', (err, data) => {
    ...
    let removedCubes = []
    for (let i = 0; i < cubes.length; i++) {
        let supportedCubes = cubes[i].supports

        let canBeRemoved = supportedCubes.length === 0 || supportedCubes.every(cube => cube.supportedBy.length > 1)

        if (canBeRemoved) {
            removedCubes.push(cubes[i])
        }
    }

    console.log(removedCubes.length)
})
