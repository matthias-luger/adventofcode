...
function getStrengthValue(hand) {
    ...
    cardTypes.forEach(cardType => {
        let amount = hand.cards[cardType]
        if (cardType === 'J') {
            jokers = amount
            return
        }
        if (amount > highestCardsAmount) {
            highestCardsAmount = amount
        }
        if (amount === 2) {
            pairs++
        }
...
function isFullHouse(hand, jokers) {
    let cardTypes = Object.keys(hand.cards).filter(cardType => cardType !== 'J')
