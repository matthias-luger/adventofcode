...
fs.readFile('./input.txt', 'utf8', (err, data) => {
    ...
    let paths = findPossiblePaths(nodesMap, 26)
    let max = 0

    for (let i = 0; i < paths.length; i++) {
        for (let j = i + 1; j < paths.length; j++) {
            if (!intersectPaths(paths[i], paths[j])) {
                if (paths[i].pressure + paths[j].pressure > max) {
                    max = paths[i].pressure + paths[j].pressure
                }
            }
        }
    }
...
function intersectPaths(pathA, pathB) {
    let intersect = false

    pathA.path.forEach(p => {
        if (pathB.path.includes(p)) {
            intersect = true
        }
    })
