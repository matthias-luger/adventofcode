...
fs.readFile('./input.txt', 'utf8', (err, data) => {
    ...
    let lowestPoint = getLowestPointOfMap(map)

    let currentMovingSand = [500, 0]
    let totalSand = 0

    while (currentMovingSand[1] < lowestPoint) {
        let hasMoved = simulateSand(map, currentMovingSand)
        if (!hasMoved) {
...
function simulateSand(map, sand) {
    if (!getMapValue(map, sand[0], sand[1] + 1)) {
        sand[1]++
        return true
    }
    if (!getMapValue(map, sand[0] - 1, sand[1] + 1)) {
        sand[1]++
        sand[0]--
        return true
    }
