...
fs.readFile('./input.txt', 'utf8', (err, data) => {
    ...
    for (let i = 0; i < 10; i++) {
        moveElfs(elfSet, moveConsiderations)
        turnMoveConsiderations(moveConsiderations)
    }

    let [minX, minY, maxX, maxY] = getRectCoordinates(elfSet)
    let area = (maxX - minX + 1) * (maxY - minY + 1)

    console.log(area - elfSet.size)
})
...
function getProposedMove(currElf, elfSet, moveConsiderations) {
    ...
    if (
        ...
        return { x: currElf.x, y: currElf.y }
    }
    ...
    return move || { x: currElf.x, y: currElf.y }
}
