...
fs.readFile('./input.txt', 'utf8', (err, data) => {
    ...
    let way1 = findWay(start, target, winds, 0)
    let way2 = findWay(target, start, winds, way1)
    let way3 = findWay(start, target, winds, way2)
    console.log(way3)
})

function findWay(start, target, winds, startStepNo = 0) {
    let queue = [{ player: { x: start.x, y: start.y }, steps: startStepNo, path: [] }]
    let visited = new Set()
