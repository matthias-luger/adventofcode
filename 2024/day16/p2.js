...
fs.readFile('./input.txt', 'utf8', (err, data) => {
    ...
    let best = result[0].currentScore

    result = result.filter(r => r.currentScore === best)
    let set = new Set()
    result.forEach(r => {
        r.path.forEach(p => set.add(p))
    })
...
function findShortestPath(map, startingPosition) {
    ...
    while (paths.length > 0) {
        ...
        if (shortestToPosMap.has(currentPosition)) {
            if (shortestToPosMap.get(currentPosition) < currentScore - 1001) {
                continue
            }
        }
